Pod::Spec.new do |s|
  s.name         = 'GBAnalytics'
  s.version      = '3.1.0'
  s.summary      = 'Abstracts away different analytics networks and provides a unified simple interface.'
  s.homepage     = 'https://github.com/lmirosevic/GBAnalytics'
  s.license      = 'Apache License, Version 2.0'
  s.author       = { 'Luka Mirosevic' => 'luka@goonbee.com' }
  s.platform     = :ios, '7.0'
  s.source       = { git: 'https://github.com/lmirosevic/GBAnalytics.git', tag: s.version.to_s }
  s.source_files  = 'GBAnalytics/*.{h,m}', 'GBAnalytics/Modules/*.{h,m}'
  s.public_header_files = 'GBAnalytics/*.h', 'GBAnalytics/Modules/*.h'
  s.requires_arc = true

  s.frameworks = 'SystemConfiguration', 'CoreData'

  s.dependency 'Fabric', '~> 1.5'
  s.dependency 'Crashlytics', '~> 3.3'
  s.dependency 'Flurry-iOS-SDK', '~> 7.3'
  s.dependency 'GoogleAnalytics', '~> 3.0'
  s.dependency 'Tapstream', '~> 2.6'
  s.dependency 'FBSDKCoreKit', '~> 4.6'
  s.dependency 'Mixpanel', '~> 2.3'
  s.dependency 'Localytics', '~> 3.0'
  s.dependency 'Parse', '~> 1.6'
  s.dependency 'Amplitude-iOS', '~> 3.2'

  s.libraries = 'z', 'c++'
end
